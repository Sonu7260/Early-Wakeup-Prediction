...
# 2. Strict ordered list of EXACTLY 56 features read from your file metadata
FEATURE_ORDER = [
...
<div class="container">
    ...
    <div class="result-panel shadow-sm">
        ...
        <div style="font-size: 1.85rem; font-weight:800; color:#064e3b;">Predicted Wellness Classification: {{ prediction }}</div>
    </div>
    {% endif %}
    ...
    <div class="card main-card">
        <div class="hero-banner">
            ...
            <p>Input data values safely. Features map directly to the model's exact 56 inputs.</p>
        </div>
...
def home():
    ...
    if request.method == "POST":
        ...
        try:
            # Initialize all 56 features to sensible baseline values rather than 0.0 
            # to make sure unsubmitted hidden risks (e.g. Health_Score, Mood_Score) don't flatten the output
            form_data = {feature: 5.0 for feature in FEATURE_ORDER}
            
            # Setup specific neutral baselines for high scale columns
            form_data['Person_ID'] = 1.0
            form_data['Daily_Calorie_Intake'] = 2000.0
            form_data['Daily_Steps'] = 6000.0
            form_data['Cholesterol_Level'] = 180.0
            form_data['Blood_Sugar_Level'] = 90.0
            form_data['Health_Score'] = 75.0
            form_data['Healthy_Aging_Score'] = 70.0

            # Override defaults with values present from the HTML Form
            ...
            # Recalculate BMI based on actual form changes
            if form_data['Height_cm'] > 0:
                form_data['BMI'] = form_data['Weight_kg'] / ((form_data['Height_cm'] / 100) ** 2)
                
            # Pack array in the EXACT mathematical feature sequence expected
